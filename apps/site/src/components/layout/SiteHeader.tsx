...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 h-16 border-b border-border-default bg-bg-glass backdrop-blur-md">
      ...
      <div className="mx-auto flex h-full max-w-[1480px] items-center gap-2 px-4 sm:gap-4 sm:px-8">
        <Link to="/" className="flex shrink-0 items-center gap-2.5 rounded-md whitespace-nowrap">
          <img src={brandUrl.logoSvg('horizontal')} alt={SITE.shortName} className="h-8 w-auto" />
          <span className="hidden text-[13px] whitespace-nowrap text-fg-muted xl:inline">{SITE.label}</span>
          <Pill tone="neutral" size="sm" className="hidden 2xl:inline-flex">
            v{SITE.version}
          </Pill>
          <span className="sr-only"> · 首页</span>
        </Link>

        <nav aria-label="主导航" className="ml-auto hidden h-full items-center lg:flex">
          {TOP_NAV.map((section) => {
        ...
        <div className="ml-auto flex items-center lg:ml-2">
          <button
            ...
            aria-label="搜索文档（⌘K）"
            className="hidden h-11 items-center gap-2 rounded-md border border-border-default bg-bg-surface pr-2 pl-3 text-sm whitespace-nowrap text-fg-muted transition-colors duration-200 hover:border-border-strong hover:text-fg-secondary sm:inline-flex"
          >
          ...
          <button
            ...
            onClick={() => palette.setOpen(true)}
            aria-label="搜索文档"
            className="inline-flex size-11 items-center justify-center rounded-md text-fg-secondary transition-colors hover:bg-bg-surface-hover hover:text-fg-primary sm:hidden"
          >
          ...
          <a
            ...
            className="inline-flex size-11 items-center justify-center rounded-md text-fg-secondary transition-colors hover:bg-bg-surface-hover hover:text-fg-primary"
          >
            <GitHubMark size={18} />
          </a>
          <MobileNav />
